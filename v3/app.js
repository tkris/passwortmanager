...
$('compareApply').onclick=async()=>{if(!s||busy||compareBusy)return;const chosen=compareRows.filter(r=>r.selected);if(!chosen.length){note('Keine Einträge ausgewählt.');return;}if(!confirm(chosen.length+' Einträge übernehmen? Abweichende Einträge im geöffneten Tresor werden ersetzt. Die zweite Datei bleibt unverändert.'))return;compareBusy=true;try{const old=s.entries;const next=old.map(e=>({...e}));for(const row of chosen){if(row.targetIndex<0){const now=Date.now();next.push({...normalizedEntry(row.source),createdAt:now,updatedAt:now});}else{const oldEntry=next[row.targetIndex],now=Date.now();next[row.targetIndex]={...normalizedEntry(row.source),createdAt:entryTime(oldEntry.createdAt)??undefined,updatedAt:now};}}s.entries=next;compareClose();await changed(old);note(chosen.length+' Einträge übernommen. Nicht gespeichert: Bitte die .enc-Datei speichern oder exportieren. '+(s.recovery?'Notfall-Zwischenstand vorhanden.':'Notfall-Zwischenstand nicht verfügbar.'),'warning');}finally{compareBusy=false;}};
