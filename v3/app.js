...
async function changed(old){pendingMaster=null;s.dirty=true;s.recovery=false;s.exportPending=false;try{await draftNow();note('Änderung verschlüsselt für die Sitzungswiederherstellung gesichert. Bitte .enc-Datei speichern.');}catch(e){note('ACHTUNG: gespeicherte Sitzung konnte nicht gespeichert werden: '+e.message+' · Änderungen existieren nur in dieser Sitzung.');}render();}
