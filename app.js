...
function isCreatingBrowserVault(){return source==='browser'&&localStorage.getItem(STORE)===null;}
...
function handleSourceSelection(type){if(busy)return;$('autoLockNotice').classList.add('hidden');source=type;$('masterPassword').value='';$('masterRepeat').value='';if(type==='file'){const file=$('importFile').files[0];if(!file)return;if(file.size>10*1024*1024){message('Datei zu groß (max. 10 MB).');return;}const reader=new FileReader();reader.onload=()=>{fileRawData=reader.result;$('promptTitle').textContent='Datei-Tresor öffnen';updateMasterPrompt();show('passwordPrompt');$('masterPassword').focus();};reader.onerror=()=>message('Datei konnte nicht gelesen werden.');reader.readAsText(file);}else{$('promptTitle').textContent=isCreatingBrowserVault()?'Neuen Browser-Tresor anlegen':'Browser-Tresor öffnen';updateMasterPrompt();show('passwordPrompt');$('masterPassword').focus();}}
...
$('backButton').addEventListener('click',()=>{fileRawData=null;$('importFile').value='';$('masterPassword').value='';$('masterRepeat').value='';show('sourceSelect');});
